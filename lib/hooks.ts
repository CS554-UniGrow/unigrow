...
export const getSessionServer = async (pathname?: string) => {
  ...
  if (user?.isAuthenticated && user?.isOnboarded && (pathname === ONBOARDING_REDIRECT || ROUTES.includes(pathname!))) {
    redirect("/dashboard")
  }

  // If user is authenticated but not onboarded, redirect to onboarding page
  if (user?.isAuthenticated && !user?.isOnboarded && pathname !== ONBOARDING_REDIRECT) {
